...
locals {
  ...
  __target_https_proxies = [for i, v in local._target_https_proxies :
    merge(v, {
      ...
      ssl_policy = try(coalesce(
        var.existing_ssl_policy,
        v.is_regional ? try(google_compute_region_ssl_policy.default[one(keys(local.ssl_policies))].self_link, null) : null,
        !v.is_regional ? try(google_compute_ssl_policy.default[one(keys(local.ssl_policies))].self_link, null) : null,
      ), null)
